...
def check_alarm(event, s, o, alarms, created, alarm_file = None, tau = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]):
   ...
   if event_type in {'execve', 'load'}:
      if o.isFile():
         # if event.id == '59819D11-6F7F-5353-B723-113206C54D62' or event.id == '59819D70-6F7F-5353-B723-113206C54D62':
         #       with open("output27.txt", "a") as f:
         #          f.write(f"{s.tags()},{citag(s.tags())},{tau},{event.id}\n")
      ...
      if 'PROT_EXEC' in set(event.parameters):
         if itag(s.tags()) < tau_s_i:
            ...
            if o:
               ...
               if not alarms[(s.get_pid(), event.type, None)]:
