...
def parse_event_cadets(node_buffer, datum, cdm_version):
    ...
    if 'ppid' in event.properties:
        if event.src not in node_buffer:
    ...
    try:
        ...
        # 设置 mmap 类型
                ...
                # 安全处理 'protection' 字段
                try:
                    event.parameters = memory_protection(eval(event.properties.get('protection', '0')))
                except KeyError as ke:
                    print(f"Warning: Missing 'protection' key in event properties: {ke}")
                    event.parameters = []  # 默认空值
                except ValueError as ve:
                    print(f"Warning: Invalid protection value: {ve}")
                    event.parameters = []  # 默认空值
