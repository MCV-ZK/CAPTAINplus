...
def start_experiment(args):
    ...
    volume_list = []
    for volume in range(100,150):
        if volume == 0:
            volume_list.append(os.path.join(args.input_data,  'ta1-trace-e3-official.json'))
        else:
            volume_list.append(os.path.join(args.input_data,  f'ta1-trace-e3-official.json.{volume}'))
            
    for volume in range(7):
        if volume == 0:
            volume_list.append(os.path.join(args.input_data, 'ta1-trace-e3-official-1.json'))
        else:
            volume_list.append(os.path.join(args.input_data,  f'ta1-trace-e3-official-1.json.{volume}'))
                ...
                if record_type == 'Event':
                    ...
                    if event:
                        if event.type == 'set_uid':
                            if event.parameters in principals_buffer:
                                if 'userId' in principals_buffer[event.parameters]:
