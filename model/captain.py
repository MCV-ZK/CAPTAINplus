...
class CAPTAIN:
    ...
    def add_event_online_loss(self,event,gt):
        diagnosis = None
        loss = 0
        s_labels = []
        o_labels = []
        ...
        dest = self.Nodes.get(event.dest, None)
        dest2 = self.Nodes.get(event.dest2, None)
        event_feature = (src.get_pid(), event.type, dest.get_name() if dest else None)
        if src:
            event_feature_str = str(dump_event_feature(event, src, dest, dest2))
            tau = self.tau_dict.get(event_feature_str, [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
            prop_lambda = self.lambda_dict.get(event_feature_str, 0)
            ...
            if o_target:
                ...
                for i, item in enumerate(o_target):
                    if item!=None:
                        ...
                        loss_thr_grads[event_feature_str][i+4] = gradients*(-1)*(1-2*item)
                        ...
                        if i == 2 and len(dest.propagation_chain['i'])>0:
                            ...
                            kill_chains.append(dest.propagation_chain['c'])
        return s_labels, o_labels, kill_chains, loss_lambda_grads, loss_thr_grads, loss
    ...
    def add_event(self, event, gt = None):                        
        ...
        if src:
            event_feature_str = str(dump_event_feature(event, src, dest, dest2))
            if event_feature not in self.alarm:
                self.alarm[event_feature] = False
            tau = self.tau_dict.get(event_feature_str, [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5])
            prop_lambda = self.lambda_dict.get(event_feature_str, 0)
            diagnosis, tag_indices = check_alarm(event, src, dest, self.alarm, self.created, self.alarm_file, tau)
            if diagnosis != None:
                self.alarm[event_feature] = True   
            propTags(event, src, dest, dest2, att = self.att, decay = self.decay, prop_lambda=prop_lambda, tau=tau)
        if diagnosis:
            self.alarm[event_feature] = True
        return diagnosis
    ...
    def update_stags(self,nid,event,gt,flag,fpcount):
        src = self.Nodes.get(event.src, None)
        dest = self.Nodes.get(event.dest, None)
        dest2 = self.Nodes.get(event.dest2, None)   
        s_target, o_target = get_target(event, src, dest, gt)  
        stagupdate=[]
        lr=0.004*(fpcount/50+1)
        if s_target!=None:
            for i in range(4):
                if len(stagupdate) <= i:
                    stagupdate.append(None)
                if s_target[i] == 0:
                    if src.tags()[i] != None:
                        if flag==0:
                            stagupdate[i]=src.tags()[i]-lr
                        if flag==1:
                            stagupdate[i]=src.tags()[i]-0.7
                        stagupdate[i]=np.clip(stagupdate[i], 0.0, 1.0)
                    else:
                        stagupdate[i]=0.0
                if s_target[i] == 1: 
                    if src.tags()[i] != None:   
                ...
                if s_target[i] == None:
                    if src.tags()[i] != None:
                        stagupdate[i]=src.tags()[i]
                    else:
                        stagupdate[i]=None

            self.Nodes[nid].setSubjTags(stagupdate)

    def update_otags(self,nid,event,gt,flag,fpcount):
        src = self.Nodes.get(event.src, None)
        dest = self.Nodes.get(event.dest, None)
        dest2 = self.Nodes.get(event.dest2, None)   
        s_target, o_target = get_target(event, src, dest, gt)  
        ...
        if o_target!=None:
            for i in range(2,4):
                if len(otagupdate) <= i-2:
                    otagupdate.append(None)
                if o_target[i] == 0:
                    if dest.tags()[i] != None:
                        if flag==0:
                            otagupdate[i-2]=dest.tags()[i]-lr
                        if flag==1:
                            otagupdate[i-2]=dest.tags()[i]-0.7
                        otagupdate[i-2]=np.clip(otagupdate[i-2], 0.0, 1.0)
                    else:
                        otagupdate[i-2]=0.0
                if o_target[i] == 1: 
                    if dest.tags()[i] != None:   
                ...
                if o_target[i] == None:
                    if dest.tags()[i] != None:
                        otagupdate[i-2]=dest.tags()[i]
                    else:
                        otagupdate[i-2]=None

            self.Nodes[nid].setObjTags(otagupdate)       
    def updateg(self,event,gt):
        src = self.Nodes.get(event.src, None)
        dest = self.Nodes.get(event.dest, None)
        dest2 = self.Nodes.get(event.dest2, None)   
        event_feature_str = str(dump_event_feature(event, src, dest, dest2))
        s_target, o_target = get_target(event, src, dest, gt)  
        if (s_target is None and o_target != None) or (s_target != None and o_target is None):
                self.lambda_dict[event_feature_str] = 0.99
